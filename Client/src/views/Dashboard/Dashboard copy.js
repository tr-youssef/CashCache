...
import { StyleSheet, Text, View, Button, useColorScheme, ScrollViewBase } from "react-native";
...
const Dashboard = ({ navigation }) => {
  ...
  const LoadPieChartData = async () => {
    ...
    if (token !== "" && token !== undefined) {
      callAPI(`/api/transactions/agg?startDate=${startDate}&endDate=${endDate}`, "GET")
        .then((res) => {
          option.series[0].data = res;
          let chart;
          if (skiaRef.current) {
            chart = echarts.init(skiaRef.current, theme === "light" ? "light" : "dark", {
              renderer: "svg",
              width: 400,
              height: 400,
            });
